...
class RpmChart extends StatefulWidget {
  ...
  const RpmChart({
    super.key,
    required this.rpmData,
    required this.timeLabels,
    this.isDark = false,
  });

  @override
  State<RpmChart> createState() => _RpmChartState();
}

class _RpmChartState extends State<RpmChart> {
  int? touchedIndex;
  double? touchedY;
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Stack(
        children: [
          LineChart(_buildRpmChartData(widget.isDark)),
            ...
            IgnorePointer(
              ...
              child: CustomPaint(
                ...
                painter: CrosshairPainter(
                  xIndex: touchedIndex!,
                  yValue: touchedY!,
                  totalPoints: widget.rpmData.length,
  ...
  /// 🔹 RPM Chart Configuration
  LineChartData _buildRpmChartData(bool isDark) {
    return LineChartData(
      minY: widget.rpmData.reduce(min) - 200,
      maxY: widget.rpmData.reduce(max) + 200,
      ...
      lineBarsData: [
        LineChartBarData(
          spots: List.generate(
            widget.rpmData.length,
            (index) => FlSpot(index.toDouble(), widget.rpmData[index]),
          ),
          isCurved: true,
          color: tOrange1,
          barWidth: 3,
          ...
          belowBarData: BarAreaData(
            ...
            gradient: LinearGradient(
              colors: [tOrange1.withOpacity(0.3), tOrange1.withOpacity(0.0)],
