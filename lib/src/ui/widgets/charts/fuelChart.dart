...
class FuelChart extends StatefulWidget {
  final List<double> fuelData; // fuel % values (0–100)
...
class _FuelChartState extends State<FuelChart> {
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Stack(
        children: [
          LineChart(_buildFuelChartData(widget.isDark)),
            ...
            IgnorePointer(
              ...
              child: CustomPaint(
                ...
                painter: CrosshairPainter(
                  xIndex: touchedIndex!,
                  yValue: touchedY!,
                  totalPoints: widget.fuelData.length,
                  isDark: widget.isDark,
                  minY: widget.fuelData.reduce(min),
                  maxY: widget.fuelData.reduce(max),
                ),
              ),
            ),
        ],
  ...
  /// 🔹 Fuel Chart Configuration
  LineChartData _buildFuelChartData(bool isDark) {
    final minFuel = max(widget.fuelData.reduce(min) - 5, 0).toDouble();
    final maxFuel = min(widget.fuelData.reduce(max) + 5, 100).toDouble();
    ...
    return LineChartData(
      ...
      lineBarsData: [
        LineChartBarData(
          spots: List.generate(
            widget.fuelData.length,
            (index) => FlSpot(index.toDouble(), widget.fuelData[index]),
          ),
          isCurved: true,
          color: tGreenDark, // 🟢 Fuel = green theme
          barWidth: 3,
          ...
          belowBarData: BarAreaData(
            ...
            gradient: LinearGradient(
              colors: [
                tGreenDark.withOpacity(0.3),
                tGreenDark.withOpacity(0.0),
              ],
...
/// 🔹 Crosshair Painter (updated for proper Y scaling)
class CrosshairPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final yRatio = (yValue - minY) / (maxY - minY);
    final yPos = size.height * (1 - yRatio.clamp(0, 1));
